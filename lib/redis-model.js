...
RedisModel.prototype.kUrl = function (url) {
  return RedisModel._prefix_ + 'url:' + this.genHash(url);
};
...
// Helpers
RedisModel.prototype.genHash = function (url) {
...
// Set record
RedisModel.prototype.set = function (long_url, datesObject, cNew, metadata, callback) {
  var self = this;
        ...
        if (err) {
          ...
          self.db.multi([
            ...
            ['hmset', self.kHash(response.hash),
              'url', long_url,
              'hash', response.hash,
              'start_date', datesObject.start_date ? datesObject.start_date.getTime() : 0,
              'end_date', datesObject.end_date ? datesObject.end_date.getTime() : 0,
              'metadata', metadata || "",
              'clicks', 0
            ]
