...
module.exports = function (opts) {
  ...
  self.shorten = function (long_url, startDate, endDate, cNew, metadata, callback) {
    if (this.checkUrl(long_url, true)) {
      this.getModel(function (err, model) {
        if (err) {
          callback(500);
        } else {
          dateObj = (startDate && endDate) ? {'start_date': new Date(startDate), 'end_date' : new Date(endDate)} : {}
          model.set(long_url, dateObj, cNew, metadata, callback);
        }
      });
    } else {
      callback(400);
